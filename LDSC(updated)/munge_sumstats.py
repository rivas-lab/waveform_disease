...
def read_header(fh):
    # Always read the first line as text, even from a .gz
    if fh.endswith('.gz'):
        with gzip.open(fh, 'rt') as f:   # 't' = text mode
            line = f.readline()
    else:
        with open(fh, 'r') as f:
            line = f.readline()
...
def parse_dat(dat_gen, convert_colname, merge_alleles, log, args):
    ...
    for block_num, dat in enumerate(dat_gen):
        ...
        wrong_types = [
            c for c in dat.columns if c in numeric_cols and not np.issubdtype(
                dat[c].dtype, np.number)]
        ...
        old = ii.sum()
        if args.keep_maf:
            dat.drop([x for x in ['INFO'] if x in dat.columns],
                     inplace=True, axis=1)
        else:
            dat.drop([x for x in ['INFO', 'FRQ']
                      if x in dat.columns], inplace=True, axis=1)
...
def allele_merge(dat, alleles, log):
    ...
    dat.loc[~jj.astype('bool'),
            [i for i in dat.columns if i != 'SNP']] = float('nan')
...
def munge_sumstats(args, p=True):
    ...
    try:
        ...
        for field in cname_translation:
            ...
            if numk > 1:
                raise ValueError(
                    'Found {num} columns named {C}'.format(
                        C=field, num=str(numk)))
        ...
        for head in cname_translation.values():
            ...
            if numc > 1:
                raise ValueError(
                    'Found {num} different {C} columns'.format(
                        C=head, num=str(numc)))

        if (not args.N) and (not (args.N_cas and args.N_con)) and ('N' not in cname_translation.values(
        )) and (any(x not in cname_translation.values() for x in ['N_CAS', 'N_CON'])):
            raise ValueError('Could not determine N.')
        if ('N' in cname_translation.values() or all(x in cname_translation.values()
                                                     for x in ['N_CAS', 'N_CON'])) and 'NSTUDY' in cname_translation.values():
            nstudy = [
                x for x in cname_translation if cname_translation[x] == 'NSTUDY']
            for x in nstudy:
                del cname_translation[x]
        if not args.no_alleles and not all(
            x in cname_translation.values() for x in [
                'A1',
                'A2']):
        ...
        if args.merge_alleles:
            ...
            merge_alleles = pd.read_csv(
                ...
                na_values='.')
            if any(
                x not in merge_alleles.columns for x in [
                    "SNP",
                    "A1",
                    "A2"]):
                raise ValueError(
            ...
            merge_alleles.drop([x for x in merge_alleles.columns if x not in [
                'SNP', 'MA']], axis=1, inplace=True)
        else:
            merge_alleles = None
        ...
        dat_gen = pd.read_csv(
            ...
            dtype={
                c: np.float64 for c in signed_sumstat_cols})
